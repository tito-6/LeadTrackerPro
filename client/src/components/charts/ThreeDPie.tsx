...
export default function ThreeDPie({
  ...
  const options: ChartOptions<"pie"> = {
    ...
    plugins: {
      ...
      datalabels: {
        display: (context: any) => {
          // Dynamic threshold based on data size and segment percentage
          const percentage = sortedPercentages[context.dataIndex];
          const totalSegments = sortedPercentages.length;

          // Adaptive threshold: more segments = higher threshold
        ...
        formatter: (value: any, context: any) => {
          ...
          // Show different info based on segment size
          if (percentage >= 20) {
            return `${label}\n${value} (${percentage}%)`;
