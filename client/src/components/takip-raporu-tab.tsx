...
import { Calendar, Phone, UserCheck, TrendingUp, AlertTriangle, FileSearch } from "lucide-react";
...
interface TakipData {
  ...
  projectName?: string;
  status: 'Arandı - Geri Dönecek' | 'Ulaşılamıyor' | 'Takip Edilecek' | 'Tamamlandı' | 'İptal';
...
export default function TakipRaporuTab() {
  ...
  const takipData = useMemo(() => {
    ...
    return leads
      .filter(lead => {
        ...
        return status.toLowerCase().includes('takip') || 
      ...
      .map((lead, index) => {
        ...
        const statusText = (lead.status || lead.lastMeetingResult || '').toLowerCase();
        
        if (statusText.includes('geri dönecek') || statusText.includes('aranacak')) {
          detectedStatus = 'Arandı - Geri Dönecek';
        } else if (statusText.includes('ulaşılamıyor') || statusText.includes('ulaşılamadı')) {
          detectedStatus = 'Ulaşılamıyor';
        } else if (statusText.includes('tamamlandı') || statusText.includes('satış')) {
        ...
        if (lead.requestDate) {
          const daysSinceRequest = Math.floor((Date.now() - new Date(lead.requestDate).getTime()) / (1000 * 60 * 60 * 24));
        ...
        return {
          ...
          gorusmeTarihi: lead.customerResponseDate || lead.requestDate || new Date().toISOString(),
          assignedPersonnel: lead.assignedPersonnel || 'Atanmamış',
          projectName: lead.projectName,
          status: detectedStatus,
          priority,
          nextActionDate: new Date(Date.now() + Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString() // Random date within next week
        } as TakipData;
      });
  }, [leads]);
  ...
  const filteredTakipData = useMemo(() => {
    return takipData.filter(item => {
      const matchesSalesperson = selectedSalesperson === 'all' || item.assignedPersonnel === selectedSalesperson;
      const matchesProject = selectedProject === 'all' || (item.projectName && item.projectName.includes(selectedProject));
  ...
  const uniqueProjects = useMemo(() => {
    const projects = new Set(takipData.map(item => item.projectName).filter(Boolean));
    return Array.from(projects);
  }, [takipData]);

  // Priority statistics
  const priorityStats = useMemo(() => {
    const high = filteredTakipData.filter(item => item.priority === 'Yüksek').length;
    const medium = filteredTakipData.filter(item => item.priority === 'Orta').length;
    const low = filteredTakipData.filter(item => item.priority === 'Düşük').length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Toplam Takip</p>
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{filteredTakipData.length}</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-red-600 dark:text-red-400">{priorityStats.high}</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Geri Dönecek</p>
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                {filteredTakipData.filter(item => item.status === 'Arandı - Geri Dönecek').length}
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">
                {filteredTakipData.filter(item => item.status === 'Tamamlandı').length}
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-red-700">Yüksek Öncelik</span>
                <span className="text-lg font-bold text-red-600">{priorityStats.high}</span>
              </div>
              <Progress value={priorityStats.total > 0 ? (priorityStats.high / priorityStats.total) * 100 : 0} className="h-2" />
              <p className="text-xs text-gray-600">Acil aksiyon gerektiren leadler</p>
            </div>
            
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-orange-700">Orta Öncelik</span>
                <span className="text-lg font-bold text-orange-600">{priorityStats.medium}</span>
              </div>
              <Progress value={priorityStats.total > 0 ? (priorityStats.medium / priorityStats.total) * 100 : 0} className="h-2" />
              <p className="text-xs text-gray-600">Normal takip süreci</p>
            </div>
            
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-green-700">Düşük Öncelik</span>
                <span className="text-lg font-bold text-green-600">{priorityStats.low}</span>
              </div>
              <Progress value={priorityStats.total > 0 ? (priorityStats.low / priorityStats.total) * 100 : 0} className="h-2" />
        ...
        <CardContent className="p-0">
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr className="border-b border-gray-200 dark:border-gray-700">
                  ...
                  <th className="text-left p-3 font-semibold text-gray-900 dark:text-gray-100">Öncelik</th>
                  <th className="text-left p-3 font-semibold text-gray-900 dark:text-gray-100">Son Görüşme</th>
                  <th className="text-left p-3 font-semibold text-gray-900 dark:text-gray-100">Sonuç</th>
              ...
              <tbody>
                {filteredTakipData.slice(0, 50).map((item, index) => (
                  ...
                  >
                    <td className="p-3 font-medium text-gray-900 dark:text-gray-100">{item.customerName}</td>
                    <td className="p-3 text-gray-700 dark:text-gray-300">{item.assignedPersonnel}</td>
                    <td className="p-3">
                      <Badge 
                        variant="outline" 
                        className={
                          item.status === 'Tamamlandı' ? 'text-green-700 border-green-300' :
                          item.status === 'Ulaşılamıyor' ? 'text-red-700 border-red-300' :
                          item.status === 'Arandı - Geri Dönecek' ? 'text-orange-700 border-orange-300' :
                    ...
                    <td className="p-3">
                      <Badge 
                        variant={item.priority === 'Yüksek' ? 'destructive' : item.priority === 'Orta' ? 'secondary' : 'outline'}
                        className="text-xs"
                      >
                        {item.priority}
                      </Badge>
          ...
          {filteredTakipData.length > 50 && (
            <div className="p-4 text-center text-gray-600 dark:text-gray-400">
              İlk 50 kayıt gösteriliyor. Toplam {filteredTakipData.length} takip kaydı bulundu.
            </div>
          )}
        </CardContent>
      </Card>
